...
GitHub 可达性强依赖**本机代理在线**：`dot_gitconfig` 中三处 `proxy` 均指向 `socks5://127.0.0.1:5376`，与 `private_dot_ssh/config` 的 `ProxyCommand` 探测端口一致（现已统一为 `5376`，历史文档中 `7890` / `5376` 混合的描述已过时，详见 [dev-tools.md](dev-tools.md)）。SSH 侧同样先探测 `127.0.0.1:5376`，在线时经 `socks5` 代理连接，离线则直连；首次启动 `zsh` 时 `zimfw` 拉取模块走 HTTPS 直连，通常不受影响。
...
- `gh` 配合 `~/.ssh/config` 的 `Host github.com → ssh.github.com:443` 与 `dot_gitconfig` 的 `socks5://127.0.0.1:5376` 代理共同保证 GitHub 可达（依赖本机代理在线，见上文「网络与代理前提」——现已统一为 `5376`，不再区分 `7890`，详见 [dev-tools.md](dev-tools.md)）
